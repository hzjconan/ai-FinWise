...
async def main() -> None:
    ...
    #    我的回答：让用户尽早看到响应，大模型输出完整信息需要时间，不用干等，提升体验。✅ 抓住了核心。
    #    完整答案（体验 + 硬工程双重理由）：
    #    - 关键指标是 TTFT（首字延迟）而非总完成时间。流式不让模型变快——总生成时间基本不变，
    #      改变的是「token 一产生就送达」而非憋到最后一次性给：实际延迟几乎不变，感知延迟断崖式下降。
    #      输出越长（代码/长文/多步 agent），差距越夸张，直接影响流失率。
    #    - 避免连接超时（硬刚需）：大 max_tokens 的非流式请求，服务器憋着不回，空闲连接会被
    #      网关/代理/SDK 判超时掐断；anthropic SDK 甚至直接拒绝大 max_tokens 的非流式请求。
    #    - 可提前中断省成本：用户读到前面发现跑偏就按停，未生成的部分不产生、不计费。
    #    - 渐进渲染 + 过程可观测：边收边渲染；agent 界面还能实时展示中间步骤（调哪个工具、思考）。
    #    - 可边生成边干预（steering）：高级 agent 场景能在生成过程中插话/纠偏。
    #    联系仓库：FinWise「对话气泡逐字冒出来」就是这条链路——anthropic_client.py 把 LLM 流事件
    #    （即本脚本看到的那些）→ chat_service 翻成 SSE → 前端逐条渲染。本脚本就是最底层那一环。
