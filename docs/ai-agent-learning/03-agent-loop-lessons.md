...
这条是收尾时一个"之前练习里要加 `tool_use_id`，这里怎么没有"的直觉，牵出的整片问题。

### 两个只在真实 API 才暴露的 bug

多步工具 loop 一直用 **MockLLMClient**（单测）和 **bridge**（`a2` 真机）跑，从没上过托管 API。这两层都很**宽容**，把两处结构错误盖了整整一个 B 系列：

1. **回喂缺 id 配对**：`tool_use` 块该带 `id`、`tool_result` 该用 `tool_use_id` 指回去（真实 API 强制）。但 `ToolResult` 事件从源头就没存 `id`（`anthropic_client` 提取时把 `block.id` 丢了），回喂时自然填不上。
2. **历史留悬空 tool_use**：`build_api_messages` 把存库的终态 tool_use（ask/conclude）回放成 `tool_use` 块，可它后面**没有配对的 tool_result**（终态工具不回喂）——真实 API 拒绝这种悬空 tool_use。修法：终态工具本质是结构化输出，回放成**纯文本**即可。

### 为什么两层替身都没报错

| 替身 | 为什么盖住 |
|---|---|
| MockLLMClient | **完全不校验消息结构**——只记 `calls`、回放脚本事件。没 id 也无所谓 |
| bridge（本地 claude CLI shim） | **宽容**，不强制 tool_use_id 配对。所以 a2 真机 search→detail→conclude "跑通了" |

### 教训：全绿 ≠ 符合真实契约

> **"测试全绿"只证明"符合我们写的断言"，不证明"符合真实契约"。** 当你依赖的是一个**宽容的替身**（mock / 本地 shim / stub），它的"方便"同时就是它的"盲区"——它不会替你检查真实那一端的强制要求。越是靠替身跑通的路径，越要主动追问："真实那一端还要求什么？"

落地做法：mock 测不出 id 配对，就**加一个显式追 id 的结构测试**（断言回喂 `messages` 里 `tool_use.id == tool_result.tool_use_id`）——这类"替身盖不住的契约"，得靠专门的断言把它钉出来，而不是指望普通用例撞上。这跟前面 provenance 那条同源：**别只信"跑通了"，要能指出证据。**
...
12. 替身（mock/bridge）宽容会掩盖真实 API 契约——全绿只证明"符合断言"，不证明"符合真实契约"。
13. 靠替身跑通的路径，主动追问"真实那端还要求什么"；替身盖不住的契约（如 id 配对）得靠专门断言钉出来。

做到能对着自己的代码把这十三条讲清楚，B 系列就真正内化了。
